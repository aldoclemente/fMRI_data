...
for(i in 1:n_sim){
  ...
  observations <- exact1  # + rnorm(nlocs,mean=0,sd=0.05*diff(range(exact1)))
...
if( !dir.exists("data/test_Cshaped2D_mono_vs_lm/")){
  dir.create("data/test_Cshaped2D_mono_vs_lm/")
}

folder.name = paste("data/test_Cshaped2D_mono_vs_lm/",date_,"/",sep="")
